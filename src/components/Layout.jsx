...
export default function Layout() {
  ...
  useEffect(() => {
    base44.auth.me().then(u => { setUserRole(u?.role); setUserEmail(u?.email); }).catch(() => {});
  }, []);
    ...
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-sidebar border-r border-sidebar-border flex flex-col transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="relative p-5 border-b border-sidebar-border overflow-hidden">
          {/* Hex decorations */}
          <svg className="absolute -right-3 -top-3 opacity-[0.07]" viewBox="0 0 60 52" width="60" height="52">
            <polygon points="30,1 59,16 59,46 30,51 1,46 1,16" fill="none" stroke="hsl(101,48%,52%)" strokeWidth="2"/>
          </svg>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 lg:p-8 pb-20 lg:pb-8 hex-pattern">
          <Outlet />
        </main>
